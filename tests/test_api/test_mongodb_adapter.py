...
def test_index_creation(mongodb_collection, unique_flag):
    ...
    existing_indexes = mongodb_collection.mongo_collection.index_information()
    if index_name in existing_indexes:
        mongodb_collection.mongo_collection.drop_index(index_name)

    # Ensure index does not exist before the test
    existing_indexes = mongodb_collection.mongo_collection.index_information()
    assert index_name not in existing_indexes, f"Index {index_name} should not exist before test"
